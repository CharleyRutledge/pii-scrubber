...
# ---------- German KVNR (health insurance number) ----------
# 20 characters: 1 letter (insurer-assigned) + 19 digits, with two nested
# Luhn checksums. The reference algorithm computes each Luhn digit over
# `int(segment[::-1])` rather than the raw digit string - converting to int
# silently drops a leading zero if the reversed segment starts with one,
# which shifts which positions the Luhn "double every other digit" rule
# lands on. That quirk is replicated deliberately (via _luhn_digits_int_roundtrip)
# rather than "corrected", since the goal is matching what real KVNRs
# (generated by this exact reference implementation) actually look like.
_DE_KVNR = re.compile(r"\b[A-Za-z]\d{19}\b")


def _luhn_checksum_digit(number: int) -> int:
    digits = [int(d) for d in str(number)]
    odd_digits = digits[-1::-2]
    even_digits = digits[-2::-2]
    total = sum(odd_digits) + sum(sum(int(d) for d in str(d2 * 2)) for d2 in even_digits)
    return total % 10


def _de_kvnr_ok(raw: str) -> bool:
    letter = raw[0].upper()
    if letter not in _ascii_uppercase:
        return False
    letter_number = f"{_ascii_uppercase.index(letter) + 1:02d}"
    full = letter_number + raw[1:]  # reconstruct the original 21-digit numeric form

    first_part, first_check, second_part, second_check = full[:10], full[10], full[11:20], full[20]
    if not (first_part.isdigit() and second_part.isdigit()):
        return False

    expected_first = _luhn_checksum_digit(int(first_part[::-1]))
    if int(first_check) != expected_first:
        return False

    without_final = first_part + first_check + second_part
    expected_second = _luhn_checksum_digit(int(without_final[::-1]))
    return int(second_check) == expected_second


# ---------- UK NHS Number ----------
# 10 digits, weighted mod-11 checksum over the first 9 (weights 10..2),
# per the official NHS Data Dictionary algorithm. A result of 11 maps to
# check digit 0; a result of 10 means the number was never validly issued.
# Verified against the NHS Data Dictionary's own published worked example
# (401 023 213 -> check digit 7) before being wired into detection.
_UK_NHS_NUMBER = re.compile(r"(?<!\d)\d{10}(?!\d)")

_NHS_WEIGHTS = (10, 9, 8, 7, 6, 5, 4, 3, 2)


def _uk_nhs_number_ok(raw: str) -> bool:
    digits = [int(c) for c in raw]
    total = sum(d * w for d, w in zip(digits[:9], _NHS_WEIGHTS))
    remainder = total % 11
    check = 11 - remainder
    if check == 11:
        check = 0
    if check == 10:
        return False  # never validly issued
    return digits[9] == check


# ---------- Ontario Health Card Number (OHIP) ----------
# 10 digits, Luhn-validated, per Ontario Ministry of Health technical
# specifications. Conventionally grouped in 4-3-3 ("1234 567 890").
_CA_ON_HEALTH = re.compile(r"(?<!\d)(?:\d{10}|\d{4} \d{3} \d{3})(?!\d)")


def _ca_on_health_ok(raw: str) -> bool:
...
_CHECKSUM_RULES = [
    ...
    ("DE_KVNR", _DE_KVNR, lambda m: _de_kvnr_ok(m.group())),
    ("CA_ON_HEALTH", _CA_ON_HEALTH, lambda m: _ca_on_health_ok(m.group())),
]
...
_DIGIT_RUN_RULES = [
    ...
    ("UK_NHS_NUMBER", _UK_NHS_NUMBER, lambda digits: _uk_nhs_number_ok(digits)),
]
